...
import { Navigation, Pagination } from "swiper/modules"; // Ensure this path matches your package version
import "swiper/css";
      ...
      <div className="relative">
        ...
        {field.images?.length > 0 ? (
          <Swiper
            ...
            loop={field.images.length > 1} // Only loop if there are at least 2 images
      ...
      <div className="p-3 bg-white rounded-b-md shadow flex justify-between items-start">
        <div>
          ...
          <p className="text-sm text-gray-600">
            {field.location || "No location provided"}
          </p>
          <p className="text-xs text-gray-500">
            Posted by: {field.hostUsername || "Unknown"}
          </p>
        </div>
        <p className="text-green-600 font-bold">
          ${field.price_per_hour || "N/A"}/hour
        </p>
      </div>
    </Link>
  );
